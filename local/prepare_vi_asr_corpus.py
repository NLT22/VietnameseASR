...
def recorder_key(name: str):
    """Sort key matching how voice recorders number takes.

    Windows Voice Recorder writes the FIRST take as "Recording.wav" and later
    ones as "Recording (2).wav". Plain natural sort puts the unnumbered file
    last, which shifts every recording against its script line by one. Treat a
    missing "(N)" as take 1.
    """
    m = re.search(r"\((\d+)\)", name)
    return (int(m.group(1)) if m else 1, natural_key(name))
...
def list_audio_files(audio_dir: Path, copy_only_wav: bool) -> List[Path]:
    ...
    files.sort(key=lambda p: recorder_key(p.name))
    if not files:
