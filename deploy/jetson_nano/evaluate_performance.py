...
def get_args():
    parser = argparse.ArgumentParser(
        description="Evaluate VietnameseASR ONNX runtime speed and WER."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
        required=True,
        help="TSV with columns utt_id, speaker, audio_path, text.",
    )
    parser.add_argument(
        "--audio-root",
        type=Path,
        required=True,
        help="Directory used to resolve relative audio_path values.",
    )
    parser.add_argument(
        "--model-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        default="cpu",
        help="ONNX Runtime provider used by sherpa-onnx. Use trt/tensorrt for TensorRT.",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=0,
        help="Evaluate only the first N utterances. 0 means all.",
    )
    parser.add_argument(
        "--warmup",
        type=int,
        default=1,
        help="Number of initial utterances to decode before timed evaluation.",
    )
    parser.add_argument(
        "--output-json",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional path to write a short Markdown report.",
    )
...
def decode_one(recognizer, samples, sample_rate):
    ...
    return normalize_text(stream.result.text), elapsed
...
def summarize(args, rows, recognizer):
    ...
    waves = []
    for row in rows:
        samples, sample_rate = read_wave(row["audio_path"])
        waves.append((samples, sample_rate))
...
def write_markdown(path: Path, result):
    ...
    lines = [
        "# VietnameseASR Performance Evaluation",
        "",
...
def main():
    ...
    m = result["metrics"]
    print(
        f"{result['config']['model_type']} threads={args.threads} "
        f"WER={m['wer_percent']:.2f}% RTF={m['rtf_wall']:.4f} "
        f"speed={m['audio_x_realtime_wall']:.2f}x "
